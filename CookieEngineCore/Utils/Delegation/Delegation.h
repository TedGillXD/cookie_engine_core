...
namespace Cookie::Util {
	...
	public:
		...
		void AddFunction(ClassType* classObjPtr, typename DynamicDelegate<ClassType, void, ArgsType ...>::FuncT funcPtr) {
			_delegatePtrs.PushBack(std::make_unique<DynamicDelegate<ClassType, void, ArgsType ...>>(classObjPtr, funcPtr));
		}

		void AddFunction(typename StaticDelegate<void, ArgsType ...>::FuncT funcPtr) {
			_delegatePtrs.PushBack(std::make_unique<StaticDelegate<void, ArgsType ...>>(funcPtr));
		}
